...
public final class OpenCameraInterface {
    ...
    public static OpenCamera open(int cameraId) {
        ...
        Camera camera;
        if (index < numCameras) {
            LogEx.i(TAG, "Opening camera #" + index);
            camera = Camera.open(index);
        } else {
            if (explicitRequest) {
                LogEx.w(TAG, "Requested camera does not exist: " + cameraId);
                camera = null;
            } else {
                LogEx.i(TAG, "No camera facing " + CameraFacing.BACK + "; returning camera #0");
