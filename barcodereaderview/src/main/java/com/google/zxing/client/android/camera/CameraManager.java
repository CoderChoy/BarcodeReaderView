...
public final class CameraManager {
    ...
    private int requestedFramingRectTopOffset;
    /**
    ...
    public CameraManager(View view) {
        this.view = view;
        this.configManager = new CameraConfigurationManager(view.getContext().getApplicationContext());
    ...
    public synchronized void startPreview() {
        ...
        if (theCamera != null && !previewing) {
            ...
            previewing = true;
            autoFocusManager = new AutoFocusManager(theCamera.getCamera(), mUseAutoFocus);
        }
    }
    ...
    /**
     * 设置摄像头的自动对焦功能. 前提：设备需要支持自动对焦，否则无效。
     */
    public synchronized void setUseAutoFocus(boolean newSetting) {
        this.mUseAutoFocus = newSetting;
        OpenCamera theCamera = camera;
        if (theCamera != null) {
            if (autoFocusManager != null) {
                autoFocusManager.stop();
    ...
    public synchronized Rect getFramingRect() {
        if (framingRect == null) {
            ...
            LogEx.d(TAG, "Calculated framing rect: " + framingRect);
        }
        return framingRect;
    }
     ...
     * @param frameTopOffset 取景框垂直反向向上的偏移量(有时候不想在正中间)
     */
    public synchronized void setManualFramingRect(int width, int height, int frameTopOffset) {
        if (initialized) {
            Point screenResolution = configManager.getScreenResolution();
            ...
            if (frameTopOffset > topOffset) {
                frameTopOffset = topOffset;
            } else if (frameTopOffset < -topOffset) {
                frameTopOffset = -topOffset;
            }
            topOffset = topOffset - frameTopOffset;

            framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset + height);
            LogEx.d(TAG, "Calculated manual framing rect: " + framingRect);
            framingRectInPreview = null;
        } else {
            requestedFramingRectWidth = width;
            requestedFramingRectHeight = height;
            requestedFramingRectTopOffset = frameTopOffset;
        }
    }
